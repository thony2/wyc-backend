...
async function audit(db, user, action, table, recordId, details) {
    try {
        await db.prepare(`INSERT INTO audit_log (user_id,username,action,table_name,record_id,details) VALUES (?,?,?,?,?,?)`)
...
module.exports = (db) => {
    ...
    router.post('/login', async (req, res) => {
        ...
        } catch(e) { console.error('[Login]', e.message); res.status(500).json({ error: 'Unexpected error' }); }
    });
    ...
    router.post('/products', requireAuth, async (req, res) => {
        try {
            const { name, category_slug, subcategory, sku, price, original_price,
                    stock_level, description, img_url, badge, badge_type, rooms,
                    durability, softness, is_featured, is_deal, is_active } = req.body;
            if (!name || price == null) return res.status(400).json({ error: 'Name and price required' });
            const result = await db.prepare(`
                INSERT INTO products (name,category_slug,subcategory,sku,price,original_price,
                    stock_level,description,img_url,badge,badge_type,rooms,
                    durability,softness,is_featured,is_deal,is_active)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                RETURNING id
            `).get(name, category_slug, subcategory||null, sku||null, price, original_price||null,
                   stock_level||0, description||null, img_url||null, badge||null, badge_type||null,
                   rooms||'[]', durability||3, softness||3, is_featured||0, is_deal||0, is_active??1);
            await audit(db, req.user, 'CREATE', 'products', result?.id, req.body);
            res.status(201).json({ id: result?.id, success: true });
        } catch(e) { console.error('[Products POST]', e.message); res.status(500).json({ error: e.message }); }
    });
    ...
    router.put('/products/:id', requireAuth, async (req, res) => {
        try {
            const { name, category_slug, subcategory, sku, price, original_price,
                    ...
                    durability, softness, is_featured, is_deal, is_active } = req.body;
            await db.prepare(`
                UPDATE products SET name=?,category_slug=?,subcategory=?,sku=?,price=?,
                    original_price=?,stock_level=?,description=?,img_url=?,badge=?,badge_type=?,
                    rooms=?,durability=?,softness=?,is_featured=?,is_deal=?,is_active=?,updated_at=NOW()
                WHERE id=?
            `).run(name, category_slug, subcategory||null, sku||null, price, original_price||null,
                   stock_level||0, description||null, img_url||null, badge||null, badge_type||null,
                   rooms||'[]', durability||3, softness||3, is_featured||0, is_deal||0, is_active??1,
                   req.params.id);
            await audit(db, req.user, 'UPDATE', 'products', req.params.id, req.body);
            res.json({ success: true });
        } catch(e) { console.error('[Products PUT]', e.message); res.status(500).json({ error: e.message }); }
    });
