...
async function getDashboard(req, res) {
    try {
        const stats = await db.query(`
            ...
            FROM leads
        `);
        const byService   = await db.query(`SELECT service_type, COUNT(*) AS count FROM leads GROUP BY service_type ORDER BY count DESC`);
        const recentLeads = await db.query(`SELECT id, name, phone, postcode, service_type, status, created_at FROM leads ORDER BY created_at DESC LIMIT 5`);
        return res.json({ success: true, data: { summary: stats.rows[0], by_service: byService.rows, recent_leads: recentLeads.rows, generated_at: new Date().toISOString() } });
    } catch (err) {
...
async function listLeads(req, res) {
    try {
        ...
        const limit  = Math.min(100, Math.max(1, parseInt(req.query.limit || '500', 10)));
        const offset = (page - 1) * limit;
        const status = req.query.status || null;
        let leadsResult, countResult;
        if (status && VALID_STATUSES.includes(status)) {
            leadsResult = await db.query(`SELECT * FROM leads WHERE status=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, [status, limit, offset]);
            countResult = await db.query(`SELECT COUNT(*) AS total FROM leads WHERE status=$1`, [status]);
        } else {
            leadsResult = await db.query(`SELECT * FROM leads ORDER BY created_at DESC LIMIT $1 OFFSET $2`, [limit, offset]);
            countResult = await db.query(`SELECT COUNT(*) AS total FROM leads`);
        }
        const total = parseInt(countResult.rows[0]?.total || 0);
        await audit(null, 'viewed', 'admin', { page, limit, status }, getClientIp(req));
        return res.json({ success: true, data: leadsResult.rows, meta: { total, page, limit, pages: Math.ceil(total / limit) } });
    } catch (err) {
...
async function getLead(req, res) {
    try {
        ...
        const auditLog = await db.query(`SELECT id, action, actor, detail, ip_address, created_at FROM audit_log WHERE lead_id=$1 ORDER BY created_at ASC`, [id]);
        await audit(id, 'viewed', 'admin', null, getClientIp(req));
        return res.json({ success: true, data: { ...leadResult.rows[0], audit_log: auditLog.rows } });
    } catch (err) {
...
async function updateStatus(req, res) {
    try {
        ...
        if (!VALID_STATUSES.includes(status)) return res.status(400).json({ success: false, error: `Invalid status. Must be one of: ${VALID_STATUSES.join(', ')}.` });
...
async function deleteLead(req, res) {
    try {
        ...
        await db.query(`UPDATE leads SET name='[Anonymised]', email=NULL, phone='[Anonymised]', ip_address=NULL, user_agent=NULL, status='spam', updated_at=NOW() WHERE id=$1`, [id]);
...
async function exportCsv(req, res) {
    try {
        ...
        let sql = `SELECT id, name, email, phone, postcode, service_type, message, room_length_m, room_width_m, flooring_type, CASE WHEN include_underlay=1 THEN 'Yes' ELSE 'No' END AS include_underlay, CASE WHEN include_fitting=1 THEN 'Yes' ELSE 'No' END AS include_fitting, estimated_cost, CASE WHEN gdpr_consent_at IS NOT NULL THEN 'Yes' ELSE 'No' END AS gdpr_consent, status, source, created_at FROM leads WHERE 1=1`;
...
async function setBooking(req, res) {
    try {
        ...
        if (booking_type && !validTypes.includes(booking_type)) return res.status(400).json({ success: false, error: 'Invalid booking type.' });
        await db.query(`UPDATE leads SET booking_date=$1, booking_time=$2, booking_type=$3, booking_notes=$4, updated_at=NOW() WHERE id=$5`, [booking_date||null, booking_time||null, booking_type||null, booking_notes||null, id]);
...
if (!month) month = new Date().toISOString().slice(0, 7);
        ...
        const unscheduled = await db.query(`SELECT id, name, phone, postcode, service_type, status, created_at FROM leads WHERE booking_date IS NULL AND status NOT IN ('won','lost','spam') ORDER BY created_at DESC LIMIT 20`);
        return res.json({ success: true, data: { bookings: bookings.rows, unscheduled: unscheduled.rows, month } });
    } catch (err) {
